...
DLL A と DLL B で `new` / `delete` の実装が異なると、環境によって問題になることがある。<br>
次の2つの変更のうち、このクロス DLL 問題を避けるものとして適切なものを選んでください。
