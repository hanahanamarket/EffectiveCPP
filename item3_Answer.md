...
```
- これは **`const TextBlock ctb` に対する `operator[]`** としては正しい（条件1・条件2を満たす）
- `ctb[i]` では `const` オーバーロードが選ばれ、戻り値が `const char&` なので `ctb[i] = 'x'` は不可（条件2）
- ただし **`TextBlock tb`（非 const）まで含めて考えるなら、これ1本だけでは不十分**。`tb[i]` でも同じ `const` 版しか使えず、`tb[i] = 'x'` まで禁止されてしまう
- そのため実際には本書どおり、`const` 版と非 `const` 版の **2つの `operator[]` を併用**するのが前提になる
...
const char& operator[](std::size_t position) const {  // ← const 版 operator[]
    return const_cast<TextBlock&>(*this)[position];   // ← non-const 版 operator[] へ
}
```
...
必要なら **`char& operator[](std::size_t position)`（non-const 版）から `const char& operator[](std::size_t position) const`（const 版）を呼ぶ**形にします。  
この向きなら、const 版が危険な経路を持たないので安全です。

**向きの整理（どちらも `TextBlock::operator[]` のオーバーロードの話です）:**
- **non-const 版** `char& operator[](std::size_t position)` **から** **const 版** `const char& operator[](std::size_t position) const` **を呼ぶ**のは、ロジックを1か所にまとめるのに有効です。
- **const 版** `const char& operator[](std::size_t position) const` **の中から** **non-const 版** `char& operator[](std::size_t position)` **を呼ぶ**（`const_cast` で non-const の `*this` に戻してから呼ぶ）は、`const` と書いた見え方と裏の動きがずれやすく、避けた方が無難です。
