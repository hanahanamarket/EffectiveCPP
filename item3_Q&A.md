...
のように、
- 関数末尾の `const` で **constオブジェクトから呼べる**
- 戻り値の `const char&` で **取得した文字を書き換えられない**

という二段構えが必要です。

**なぜか:**  
関数だけ `const` でも、戻り値が `char&` だと、呼び出し側は返された参照経由で中身を書き換えられてしまいます。  
つまり **「constを付けたのに、返り値で穴が空く」** 状態になります。
