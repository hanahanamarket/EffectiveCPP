...
<a id="item3-q5"></a>
#### Q5 次の `TextBlock` に `operator[]` を追加することを考える。
実装は `text[pos]` にアクセスするものとする。
```cpp
class TextBlock {
public:
    // ここに operator[] を宣言する（どれが適切かは A〜D）
private:
    std::string text;
};
```
...
```cpp
A:
char& operator[](std::size_t pos) const;  // 実装は text[pos] を返す想定
```

```cpp
B:
const char& operator[](std::size_t pos);  // 実装は text[pos] を返す想定
```

```cpp
C:
char& operator[](std::size_t pos);  // 実装は text[pos] を返す想定（const 版なし）
```

```cpp
D:
const char& operator[](std::size_t pos) const;  // const TextBlock 向け
char& operator[](std::size_t pos);              // 併設の例（non-const 用）
```
...
<a id="item3-q6"></a>
#### Q6 外から見て不変を保ちながら、内部キャッシュだけ更新するには？
...
<a id="item3-q7"></a>
#### Q7 同じロジックを2回書かずに、どちらを本体にすべきですか？
...
<a id="item3-q11"></a>
#### Q11 `const_iterator` は何が `const` ですか？
...
**一言でいうと:**  
**「何も変えたくないなら `const char* const`」**

**記載根拠（日本語版）:**  
- **p.8**: `const char * const p = greeting;` に対して「// ポインタはconst」「// データもconst」と明記されている。  
...
**ひとこと:** **メンバ末尾の `const` と、戻り値の `const char&` の両方**が必要です。A は戻り値で穴が空き、B は `const` オブジェクトから呼べず、C は条件1を満たしません。

つまり次の形が `const TextBlock` 向けの核になります。
...
**意図:**
- constオブジェクトを本当に読み取り専用にする
- 書き換え可能性を返り値で漏らさない
- non-const版と const版を使い分ける
...
しかしキャッシュは、**外から見た意味を変えずに内部だけ更新したい**代表例です。  
そのため、
...
のように `mutable` を付けて、**constメンバ関数の中でも更新可能**にします。

**ここでの本質:**
- **bitwise constness**: ビット単位で本当に一切変えない
- **logical constness**: 外から見た意味が変わらなければよい

`mutable` は、**logical constness を実装するための道具**です。

**一言でいうと:**  
**「利用者から見て不変なら、内部キャッシュ更新は `mutable` で許可する」**
